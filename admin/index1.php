				...
				<div class="container-fluid p-0">
					<div class="row">
					    ...
					    <div style="clear: both; margin-top: 50px;">
					        <button type='button' class='btn btn-primary' onclick='window.location.href="index1_detail.php?id=-1";' id='submitButton'>添加</button>
					    </div>
					</div>
				</div>
			</main>
...
<script>
	...
	function deleteThis(id) {
		...
		$.post('save_case.php', {del_id: id}, function(a,b) {
			if (a == 'success') {
	...
	function setHidden(id, ele) {
		if (ele.checked)
			$.post('hide_case.php', {id, hiddenValue: 1}, function(a,b) {
				if (a != 'success') { alert("失败！"); ele.checked = false;}
			});
		else 
			$.post('hide_case.php', {id, hiddenValue: 0}, function(a,b) {
				if (a != 'success') { alert("失败！"); ele.checked = true;}
			});
	}
